...
Anyone who's been to a digital humanities conference knows the problem with network graphs: they all look the same. A hairball of nodes and edges, maybe with some colour coding, generated by Gephi or NetworkX with default settings. They're technically correct and almost impossible to read. You can't tell what the graph is arguing because the layout isn't designed to argue anything — it's just physics simulation.

Claude Code also built the visualizations for the leather project — and the most important one was a research tool, not a presentation.
